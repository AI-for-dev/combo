 ...
 * the keys of each are the lists at the top of `node.ts`, which refuses a key
 * valid on another kind too. Resolving names against the catalogue happens in
 * `check.ts`, still before any spawn.
 */

import { parseFrontmatter } from "@earendil-works/pi-coding-agent";
import { FaultList, type Fault } from "./fault.ts";
import { rankOf, readNodes, type FlowNode } from "./node.ts";
...
export type FlowFile = {
	...
	/** The prose of each `agent` node, by id. */
...
export function readFlow(content: string, file: string): ReadFlow {
	...
	const { nodes, refused, ids } = readNodes(keys.nodes, "nodes", faults);
	...
	for (const id of sections.keys()) {
		if (!nodes.some((node) => node.id === id) && !refused.has(id)) faults.unknown("section-unknown", id, id, nodes.map((node) => node.id), "node ids");
	}
	for (const node of nodes) {
		if (node.kind === "agent" && !sections.has(node.id)) faults.add("section-missing", node.id, `\`${node.id}\` is an agent node and has no \`## ${node.id}\` section`);
	}

	// A broken input is refused once: what reads it is not reported again.
	if (input === undefined) refused.add("input");
	const rank = (fault: Fault) => position(Object.keys(keys), ids, fault);
