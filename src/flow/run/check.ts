...
export type CheckingRun = {
	...
	/** Runs the script of `node` for the visit `path`; aborting `signal` kills it. */
...
export async function visitCheck(run: CheckingRun, node: CheckedCheckNode, path: string, here: Here): Promise<Visited> {
	const outcome = await run.check(node, path, AbortSignal.any([run.signal, here.cut]));
