...
export type DocModule = {
	...
	page: string;
	/** Where a reader is sent to read the file itself. */
	source: string;
...
/** What stands in for a body, an initialiser or a member list left out. */
const ELIDED = "/* … */";

/**
 * Where "Source: …" points: the repository, at `main`.
 *
 * A relative path is shorter and is what this used to emit, but the site
 * publishes `docs/` on its own - `../../../src/agent.ts` is a dead link on every
 ...
 *
 * What is cut is cut with `ELIDED`, a comment, so that every signature on every
 * page is still TypeScript a lexer accepts. A bare `…` is not: it fails
 * highlighting, and the documentation site treats a warning as an error.
 */
export function signatureOf(text: string, sf: ts.SourceFile, st: ts.Statement): string {
	...
	if (ts.isFunctionDeclaration(st)) {
		...
		return `${text.slice(start, cut).trim()}${st.body ? ` { ${ELIDED} }` : ""}`;
	}
	if (ts.isClassDeclaration(st)) {
		...
		const members = st.members
			...
			.map((m) => {
				...
				return `\t${text.slice(m.getStart(sf), to).trim().replace(/;$/, "")}${body ? ` { ${ELIDED} }` : ";"}`;
			});
		return [header, ...members, "}"].join("\n");
	}
	if (ts.isVariableStatement(st)) {
		const lines = text.slice(start, st.end).trim().split("\n");
		// Its head and its tail: the tail is what closes the initialiser, and
		// dropping it would leave a bracket open - unreadable, and unlexable.
		return lines.length > 6 ? `${lines[0]}\n\t${ELIDED}\n${lines.at(-1)}` : lines.join("\n");
	}
...
export function renderModule(module: DocModule): string {
	...
	const lines = [GENERATED, "", `# \`${title}\``, "", `Source: [\`${module.file}\`](${module.source})`, ""];
...
export function renderIndex(modules: DocModule[]): string {
	const lines = [
		...
		`library, in [the guide](${fromApi("docs/index.md")}); this is the exhaustive surface.`,
		"",
	...
	return `${[...lines, "", ...toctree(modules)].join("\n")}\n`;
}

/**
 * The same pages again, for Sphinx: the table above is a link table, which
 * `docs/` renders as prose and the site cannot navigate. Hidden, because the
 * table is the better index for a reader and two of them on one page is one too
 * many; generated here rather than globbed in `docs/index.md`, so a module that
 * disappears leaves the navigation with it.
 */
const toctree = (modules: DocModule[]): string[] => [
	"```{toctree}",
	":hidden:",
	"",
	...modules.map((module) => relative(DOCS_DIR, module.page).split("\\").join("/").replace(/\.md$/, "")),
	"```",
];
