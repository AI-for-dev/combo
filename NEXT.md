...
eight combinators (`chain`, `fanOut`, `loop`, `reduce`, `route`, `orchestrate`,
`interview`, `swarm`), four reporters (herdr, TUI, console, silent), the session
export, **flows** (a task graph written in YAML and Markdown, discovered in
`~/.pi/agent/flows` and `.pi/flows`, checked whole before it runs), and a
generated and test-enforced `docs/`. A subagent whose own definition asks for it **delegates in turn**, two
...
/run build add a slugify helper with tests   # in a throwaway repository with .pi/checks/test.sh
...
`flows/` and asks for them at the lowest priority, so `pi -e
...
The tarball is now explicit: `files` lists `src`, `extension`, `pane`,
`agents`, `flows`, `README.md` and `docs`, with `test/` and
...
`test/flow-shipped.test.ts` fails if `files` ever stops listing `agents` or
`flows`, which would break `/run build` for every installed user while every
...
- **`/stop` cannot be typed while a command of ours is running.** pi processes
  ...
  during `/run` or `/step` the keys are the only way in. That is why
...
## 12. The flow format - built

A flow is a task graph in YAML + Markdown (branches, parallel forks, bounded
loops, human nodes, sub-flows) that replaced the linear pipeline. It was built
in `src/flow/` beside `src/pipeline/`, one PR per layer, each tested offline and
...
flows. Then the switch, split into PRs: the package exports the flow API and
`/flows` replaces `/pipelines` (done); `/run` runs flows, with the live view,
`/run resume` and the measurement, and `/build` goes (done); the `subagent`
tool and `/step` take flows (done); the linear pipeline is removed (done); the
tutorials are recaptured. [Flows: a closed language](docs/decisions.md#flows-a-closed-language)
says why the old "no YAML DSL" line moved.

Done so far: conditions (`src/flow/condition/`), schemas (`src/flow/schema.ts`), the file with `checkFlow` for `agent`, `choice`, `parallel`, `map` and `loop` nodes, and `loadFlowCatalogue`, which reads flows and agents from disk, keeps broken agent files with their cause, and checks each agent's skills. The runner (`src/flow/run/`) walks every kind of node: the composed turn, typed outputs through `submit`, `agent-from`, memory scopes, `retry:`, `timeout:`, failures and stops, visit events; `parallel` and `map` with `fail-fast` and `too-many`; `loop` with `until`, `give-up`, `carry` and `previous`; ledgers and `verdict:` nodes through the review record. `dryRunFlow` runs it on scripted sessions, keyed by address or exact visit path. `checkRun` is the run stage: `runFlow` takes the `CheckedRun` it returns, holding the tree, the ports and each check script's content. The `check` node runs a project script through the `check` port (`bashCheck` in `src/verify.ts`), and a dry run answers it from the script. The `git` port (`gitPort` in `src/git/port.ts`) is how the runner reaches git: the `commit` node commits on the run's own branch, `diff` reads a node's tree with untracked files and without touching the index, and `copies: true` runs each branch in a copy made from a snapshot of the tree, landing the patches in branch order with `landed` and `refused` in each branch's entry. `checkRun` is async, since it asks the port whether `cwd` is a repository. The `ask` node puts a question through the `ask` port (`AskUser`, grown an optional `Asking` argument): a choice card, a yes or no or a free text, its reads shown above it, one card at a time across branches, `default:`, `enough:` and `timeout:` for nobody answering, and `unattended-ask` at the run stage. Literal options type `answer` as an enum, and an `ask-from` answer compares to no literal (`condition-free-string`). The extension's card does not take `Asking` yet. The `flow` node calls another flow of the catalogue, nearest file first: the callee is checked whole with its caller (`broken-flow`, `call-cycle` with its path, `flow-input-mismatch`), only `input` goes in and the last root node's output comes back, its memory scopes are its own, and it runs through the run's ports. The copies, commit and run-stage rules read through calls, at the call path. `model:` and `timeout:` are read from the callee outward, and a dry run scripts a call whole or walks into it. `runFlow` takes an optional `runDir`: before the first node it receives the snapshot (`snapshot.json`, with every flow file reached, each named agent as parsed, each check script, the input and the settings, and each agent's skills copied under `agents/<agent>/skills/`), and the runner appends each fact to `journal.jsonl` through a journal port: an ended visit, a `carry`, a `map`'s list, an obligation raised or closed, a copy opened or landed, the run's branch, the run's end. A dry run hands the same entries back as an array. `checkFlow` exposes what it read as the checked flow's `sources`; `readSnapshot` gives back a catalogue that checks the same, and `readJournal` ignores a torn last line. `resumeFlow(runDir, ...)` carries a run on from its snapshot, checked again with the scripts it started with, under the lock (`lock.json`, pid and host): every visit that ended with a value survives and is not visited again, an answered `ask` included, and the first that did not end runs, inside an open loop, `map` or call, with `carry`, frozen lists and ledgers restored and fresh subagents. A failed run replays its failure chain, unless the flow decided the failure (a cap, `give-up`, a condition, `too-many`, `empty-message`). Only `timeoutMs` may be given again. `HEAD` must be on the run's branch; a copy left open is taken back, and one gone or never landed starts its branch over (`copy_lost`). `resumePoint(checked, journal)` is the shared reading, and `dryRunFlow` takes `from:` a journal. A checked flow carries its `bounds`, the worst case in agent turns and time per node and in total, sub-flows unrolled (`src/flow/bounds.ts`). `src/flow/render/` draws it: `planOf` and `showPlan`, the text plan keyed by visit path, and `mermaidOf`, a `flowchart`; `npm run docs` writes `docs/reference/flows/`, one page per shipped flow. `livePlan(checked, journal, events)` is the live view: the plan filled from the earlier lives' journal and this life's events, a running visit expanded, an ended one a line, `choice` cases folded, unreached nodes marked `–`, an `ask` waiting `blocked`, and a summary with the failed visits, the loops that did not converge, the cost over every life, the lives and where a resume picked up; `showLive` and `showSummary` write it at the caller's width, and `liveRows` hands its lines to a caller that colours them. The shipped flows are in `flows/`, symlinked into `.pi/flows/` and in the package's `files`: `explore`, `split`, `interview`, `build` (whose `check` is `.pi/checks/test.sh`, this repository's running `npm test`) and `build-attended` (interview, confirm, build, commit). Each is checked against the shipped agents, walked with `dryRunFlow` in `test/flow-shipped.test.ts` and `test/flow-shipped-build.test.ts`, and drawn in `docs/reference/flows/`. A run directory holds the exports too: each subagent leaves `<home>/<agent>.jsonl` and `.html`, its home being its memory scope's path or its visit's, a taken name taking the first free `~n`, and an agent whose `tools:` names `subagent` is handed the tool, its children in `<parent>.children/`. The journal opens each life with a `life_start`, and a `visit_end` carries its `node`, `kind` and `subagent`. A `measuredRun` on the run directory adds `visits`, `nodes` and `lives` to `usage.json`, `total` summing the lives, a life killed before it wrote its own rebuilt from the journal as `partial`, and `events~n.jsonl` and `main~n.jsonl` per later life (`src/measure/lives.ts`, `flow-usage.ts`). The live summary reads the lives from the journal alone. The package root exports the flow API, and `/flows` lists every flow with its source, bound and description, refused files beside them with their faults, each file of the user's or the repository's `pipelines/` among them as `pipeline-format-removed`; `/flows <name>` prints the plan, lines cut to the terminal. Tutorial 04 still quotes `/pipelines` until its recapture. `/run <flow> <input>` runs a flow in pi: `checkFlow`, `checkRun` with the terminal's card, `bashCheck` and `gitPort`, then `runFlow` in `runs/<timestamp>/` under a `measuredRun` with the parent session; the widget is a component drawing the plan, sixteen rows at most cut around what runs now, a running visit's subagents under it; the answer lands as a message that triggers no turn, the output, one line on how it ended, and the last frame in its details. `--model` and `--timeout` lead or end the line. `/run` alone lists the flows; `/run resume` takes the newest run here that can go on (`latestResumable`) and names it and its visit, `/run resume <path>` that one. `/build`, its flags and `build.json` are gone from the extension, `/run build` is the shipped build, and a flow cannot be called `resume` (`reserved-name`). herdr names a flow's split `agent @ home`, from the `home` a spawn event now carries. Checked in a real pi on `ilaas/gemma-4-31b`: `/run explore` in this repository, `/run build` in a throwaway repository killed during its audit, and `/run resume` finishing it. `/step` takes a flow as a stage, flows first, `--agent` forcing the agent, in the step's folder as its run directory, a stage that stopped saying what `/run resume <folder>` would do; `--worktree` is gone from it, and the relay writes a step's input itself. The `subagent` tool runs a flow with `flow` and `task`, refusing any other field but `model`, `timeoutMs`, `scope` and `herdrAll` by name; its `ask` cards are shown during the model's turn through `ctx.ui`, `ctx.hasUI` being whether somebody is there, and the model reads the output and the line naming the run directory. `/run`, `/step` and the tool launch through `extension/commands/launch.ts`, and nothing in the extension reads `src/pipeline/` any more. The widget hides its `esc` hint while a card is up. Checked in a real pi on `ilaas/gemma-4-31b`: the model ran `interview` through the tool, a card answered with `enter`, one declined with `esc` (the card's "enough", not pi's interrupt), one answered through `Other…` and typed, then `/step explore`, `/step scout` and the tool refusing `steps` beside `flow`. The linear pipeline is gone: `src/pipeline/`, `pipelines/`, `docs/guide/pipelines.md`, the `deliver`, `pair` and `audit` combinators with `settle` and `build.json`'s `resume.ts`, and `Verify` with `commandVerifier`; `examples/11-build.ts` runs the shipped `build` flow, and `/flows` still lists what is left in an old `pipelines/` directory, without any parser. Left: in pi's RPC mode `hasUI` is true while `custom()` returns `undefined`, so a card there is declined at once; two tool calls at once could each put a card up, since the tool is not `executionMode: "sequential"`; the tutorials' recapture; `build`'s answer is its `deliver` loop's whole record as JSON, since that is its last root node, which a closing node in the flow would say better; `main.jsonl` is missing from a run started in a pi session that has not written its file yet; and a real run of `build-attended`, whose interview and confirm card no real run has shown yet.
...
- **A known flag written after the name was prose, silently.** Measured on
  2026-09-19:
  ...
  repository for `ilaas/mistral-small-4-119b`. `/run` now reads `--model` and
  `--timeout` at the end of the line too (`parseFlags`); in the middle of the
  text they stay text. `/step`, `/swarm` and `/interview` still read leading
  flags only.
