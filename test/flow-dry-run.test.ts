...
describe("a dry run", () => {
	...
	test("refuses a script before the start, with every fault in it", async () => {
		...
		assert.deepEqual([run.faults[0]?.message, run.faults[4]?.message], ["`pla` names no agent or check node: did you mean `plan`?", "`look` names no agent or check node: did you mean `gate/look`?"]);
	});
	...
	test("refuses a visit past a bound, a list longer than the visits it can answer, and a path that leads nowhere", async () => {
		...
		assert.deepEqual(
			...
			[
				...
				"answer-unknown-node deliver#1/both/look: `deliver#1/both/look` names no agent or check node: a visit inside `both` names its branch: left, right",
				"answer-unknown-node deliver#1/both/middle/look: `deliver#1/both/middle/look` names no agent or check node: a visit inside `both` names its branch: left, right",
				"answer-unknown-node deliver/audit#1: `deliver/audit#1` names no agent or check node: a visit path numbers each loop iteration `#n` and each map item `[i]`, and nothing else",
